...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        n_downsampling = 2
        for i in range(n_downsampling):  # 下采样层（插入EBlock）
            mult = 2 ** i
            # 关键修改：下采样前插入EBlock
            model += [EBlock(c=ngf * mult, DW_Expand=2, dilations=[1, 2], extra_depth_wise=False)]

            if (no_antialias):
                # 无抗锯齿：直接Conv2d下采样（stride=2）
                model += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                # 有抗锯齿：Conv2d(stride=1) + DownSample
        ...
        for i in range(n_blocks):  # ResNet blocks
            model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                                  use_bias=use_bias)]

        for i in range(n_downsampling):  # 上采样层
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [
            SNConv2d(dim, dim, kernel_size=3, padding=1, bias=use_bias),
            norm_layer(dim),
            nn.ReLU(True)
        ]
