        ...
        if epoch % 5 == 0:
            ...
            if iscycle:
                for i, data_val in enumerate(dataset_val):
                    ...
                    #ssim_avg[1, epoch - 1, i] = ssim(fake_im_A.squeeze().squeeze(), real_im_A.squeeze().squeeze(),data_range=1)
                ...
                for i, data_val in enumerate(dataset_val):
                    ...
                    #ssim_avg[epoch - 1, i] = ssim(fake_im_B.squeeze().squeeze(), real_im_B.squeeze().squeeze(),data_range=1)
                    ssim_scores = []
                    for idx in range(fake_im_B.shape[0]):
                        fake_single = fake_im_B[idx]  # 形状：(1, 256, 256)
                        real_single = real_im_B[idx]  # 形状：(1, 256, 256)
                        fake_np = fake_single.squeeze(0)  # 直接得到numpy数组，无需detach/cpu
                        real_np = real_single.squeeze(0)
                        ssim_val = ssim(
                            fake_np,
                            real_np,
                            data_range=1,          
                    ...
                    ssim_avg[epoch - 1, i] = np.mean(ssim_scores)
